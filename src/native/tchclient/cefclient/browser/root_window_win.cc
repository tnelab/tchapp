...
void RootWindowWin::RegisterRootClass(HINSTANCE hInstance,
  ...
  ::GetModuleFileName(0, icon_path,MAX_PATH);
  std::wstring icon_path_str(icon_path);
  icon_path_str.erase(icon_path_str.size() - 3, 3).append(L"ico");
  wcex.hIcon = (HICON)LoadImage(hInstance, icon_path_str.c_str(), IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
  //zmg end

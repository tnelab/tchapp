...
void TchCairoRenderer::Render() {
  ...
  /*cairo_t *cr = gdk_cairo_create(glarea_->window);
  cairo_set_source_rgba(cr, 1, 1, 1, 0.1);
  cairo_rectangle(cr, 0, 0, 900, 500);
  cairo_fill(cr);*/
}
...
void TchCairoRenderer::SetSpin(float spinX, float spinY) {
  spin_x_ = spinX;
  spin_y_ = spinY;
}

void TchCairoRenderer::IncrementSpin(float spinDX, float spinDY) {
  spin_x_ -= spinDX;
  spin_y_ -= spinDY;
}
